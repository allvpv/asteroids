...
// The main advantage of this algorithms is it's simplicity, so it is easy to
// implement it at 2 AM.
inline bool intersect(const ObjectContour& lhs, const ObjectContour& rhs,
    ...
    bool no_intersection_on_x = fabsf(distance.x) > lhs.half_of_sides.x + rhs.half_of_sides.x;

    if (no_intersection_on_x)
        return false;

    bool no_intersection_on_y = fabsf(distance.y) > lhs.half_of_sides.y + rhs.half_of_sides.y;

    if (no_intersection_on_y)
        return false;
